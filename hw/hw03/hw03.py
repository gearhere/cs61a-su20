...
def g_iter(n):
    ...
    if n in (1, 2, 3):
        ...
        i, last1, last2, last3 = 4, 3, 2, 1
        while i <= n:
            result = last1 + 2 * last2 + 3 * last3
            last1, last2, last3 = result, last1, last2
            i += 1
        return result
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def find_max(total, m):
        if total < 2 ** m:
            return m - 1
        else:
            return find_max(total, m+1)

    def helper(total, exp):
        # exponent
        if exp == 0:
            return 1
        elif total == 0:
            ...
            with_coin = helper(total - 2 ** exp, exp)
            without_coin = helper(total, exp - 1)
            return  with_coin + without_coin
    return helper(total, find_max(total, 0))
